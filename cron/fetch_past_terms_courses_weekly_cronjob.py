...
fetch_courses_weekly_cron_blueprint = Blueprint('fetch_past_terms_courses_weekly_cronjob', __name__, url_prefix="/api")

# Takes all the requirements satisfied + those not satisfied , and fetches all the data
@fetch_courses_weekly_cron_blueprint.route('/fetch_past_terms_courses_weekly_cronjob', methods=['POST'])
def fetch_courses_weekly():
    print("Fetching courses weekly run started")
    ...
    current_terms = [term for term in terms if '(View Only)' in term['description']]
